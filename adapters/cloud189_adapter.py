...
-----END PUBLIC KEY-----"""
    ...
    def __init__(self, cookie: str = "", index: int = 0):
        ...
        self._cookie_kv = self._parse_cookie_kv(cookie)
        self._user_name = self._cookie_kv.get("username") or self._cookie_kv.get("userName") or ""
        ...
        self._account_name = self._cookie_kv.get("name") or self._user_name or f"cloud189用户{self.index}"
        self._debug = (
    ...
    def init(self) -> Any:
        ...
        try:
            ...
            raise RuntimeError("cloud189 登录需要验证码，请在 WebUI 点击“验证码登录”完成登录")
        except Cloud189SecondValidRequired:
            raise RuntimeError("cloud189 登录需要二次设备校验，请在 WebUI 点击“验证码登录/盾牌按钮”完成登录")
